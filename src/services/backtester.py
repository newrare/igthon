...
This is the real-data counterpart of :mod:`src.services.simulator`. Where the
simulator feeds synthetic curves, the backtester feeds candles read from the
on-disk archive (:mod:`src.services.backtest_archive`). Both share the very same
replay engine (:class:`~src.services.simulator.StrategySimulator`), so a backtest
...
from src.services.simulator import SimulationConfig, SimulationResult, StrategySimulator
from src.services.trading import TradeConfig
...
def run_backtest(
    ...
    """Replay ``strategy_name`` over archived candles and return the result.

    The strategy is resolved by name through the registry; when omitted the
    configured ``STRATEGY_NAME`` is used, so the backtest replays exactly what
    the live bot would do on that historical data.
    """
