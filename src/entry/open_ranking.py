...
Unlike the per-epic breakout entries (``open_donchian`` / ``open_projection``),
...
Documented in ``docs/strategies/open_ranking.md``.
"""
...
@dataclass
class OpenRanking(EntryStrategy):
    ...
    @classmethod
    def from_settings(cls, settings) -> OpenRanking:
