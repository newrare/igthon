# `donchian_er` — Donchian breakout gated by trend efficiency

**Status:** live default (`STRATEGY_NAME=donchian_er`).

- Code: [src/strategies/donchian.py](../../src/strategies/donchian.py)
- Research origin: [research-lab.md](research-lab.md) (best of 5 candidates),
  refined with the regime-gate sweep below.
...
| ------------------------------ | ------------- | ------------------------------------- |
| `STRATEGY_NAME`                | `donchian_er` | Selects this strategy                 |
...
trends (see [research-lab.md](research-lab.md)). Pure sideways is the worst
case; real markets alternate regimes, which is why `mixte` (every seed
positive) is the honest proxy for live expectation.

Reproduce: `/simulator` page (pick `donchian_er`), or
`run_simulation(settings, SimulationConfig(profile=..., seed=...))`.

### Research lab (long + short) — exploratory, single seed, high variance

The lab's headline numbers came from one favourable seed (12345) and are **not
representative**: on `sideways` the lab's P&L flips sign across seeds (seed
12345 +386 €, seed 7 −472 €, seed 42 −184 €). Only the *trending* profiles are
sign-stable. Treat lab figures as directional intuition, never as expected
value. Reproduce the seed sweep:
...
1. **One-shot churn**: in a strongly trending day the strategy may re-enter
   ...
   this, but a per-epic daily entry cap could be added.
