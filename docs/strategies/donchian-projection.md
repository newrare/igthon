# `donchian_projection` — Donchian breakout confirmed by a multi-model projection

**Status:** opt-in entry (`ENTRY_STRATEGY_NAME=donchian_projection`).

- Code: [src/entry/donchian_projection.py](../../src/entry/donchian_projection.py)
- Projection maths: [src/core/projection.py](../../src/core/projection.py)
- Builds on: [donchian-er.md](donchian-er.md)

## Idea

`donchian_er` opens a breakout as soon as the regime gate (Kaufman Efficiency
...
`CloseProfile` (reference pairing: `atr_trailing`).
...
1. **Regime gate** — Kaufman Efficiency Ratio over `STRATEGY_EFFICIENCY_PERIOD`
   candles must reach `STRATEGY_MIN_EFFICIENCY` (identical to `donchian_er`).
...
([src/entry/donchian_projection.py](../../src/entry/donchian_projection.py)), not
