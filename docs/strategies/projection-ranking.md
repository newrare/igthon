# `projection_ranking` — cross-epic ranker, keep a rolling position all day

**Status:** opt-in entry (`ENTRY_STRATEGY_NAME=projection_ranking`).

- Code: [src/entry/projection_ranking.py](../../src/entry/projection_ranking.py)
...
The per-epic entries (`donchian_er`, `donchian_projection`) watch every market
...
`CloseProfile` (reference pairing: `atr_trailing_profit`).

## Where the decision lives

| Concern                              | Owner                                      |
| ------------------------------------ | ------------------------------------------ |
| Per-epic "how promising?" score      | `ProjectionRankingEntry.evaluate` (entry/) |
...
falls back to the shared risk gates only (`max_positions`, `euro_loss_max`). The
daily P&L circuit-breakers (`STRATEGY_DAILY_WIN_TARGET` /
`STRATEGY_DAILY_LOSS_LIMIT`) still apply: once hit, no new position is opened even
though the day's last one has closed.

## Configuration

All parameters are **constants in the strategy class**
([src/entry/projection_ranking.py](../../src/entry/projection_ranking.py)), not
`.env`/`Settings`: tune them by editing the class, and **select the strategy at
runtime from the dashboard**. The dataclass-field defaults:
...
- This supersedes the legacy `src/strategies/trend_template.py` (same cross-epic
  idea, pre-decoupling style) in the decoupled `entry/` + `exit/` model.
