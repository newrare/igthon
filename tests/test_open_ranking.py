...
from src.entry import EntryIntent, OpenRanking, get_entry_strategy
...
class TestRegistry:
    def test_known_name_resolves(self):
        strat = get_entry_strategy("open_ranking", _settings())
        assert isinstance(strat, OpenRanking)

    def test_is_cross_epic_selection(self):
        assert OpenRanking.cross_epic_selection is True

    def test_is_entry_strategy_instance(self):
        assert isinstance(OpenRanking(), EntryStrategy)
...
class TestEvaluate:
    def test_rising_curve_emits_buy_with_bounded_score(self):
        intent = OpenRanking().evaluate("TEST.EPIC", _buffer(_trending_up()))
    ...
    def test_insufficient_warmup_returns_none(self):
        ...
        assert (
            OpenRanking().evaluate("TEST.EPIC", _buffer(_trending_up(40)))
            is None
        )
