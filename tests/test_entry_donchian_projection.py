...
(src/entry/donchian_projection.py).

The strategy reuses the donchian_er gates (spread, efficiency regime, breakout)
...
from src.entry import DonchianProjectionEntry, EntryIntent, get_entry_strategy
...
class TestRegistry:
    def test_known_name_resolves(self):
        strat = get_entry_strategy("donchian_projection", _settings())
        assert isinstance(strat, DonchianProjectionEntry)
    ...
    def test_parameters_are_class_constants(self):
        ...
        custom = DonchianProjectionEntry(
...
class TestEvaluate:
    def test_breakout_with_confirming_projection_emits_buy(self):
        intent = DonchianProjectionEntry().evaluate(
    ...
    def test_breakout_rejected_when_projection_score_too_high_to_reach(self):
        ...
        strat = DonchianProjectionEntry(min_projection_score=1.01)
    ...
    def test_insufficient_warmup_returns_none(self):
        ...
        assert (
            DonchianProjectionEntry().evaluate("TEST.EPIC", _buffer(_trending_up(30)))
            is None
        )

    def test_is_entry_strategy_instance(self):
        assert isinstance(DonchianProjectionEntry(), EntryStrategy)
