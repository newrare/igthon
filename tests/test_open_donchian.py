"""Tests for the decoupled Donchian *entry* strategy (src/entry/open_donchian.py).
...
from src.entry import EntryIntent, OpenDonchian, get_entry_strategy
...
class TestRegistry:
    def test_known_name_resolves(self):
        strat = get_entry_strategy("open_donchian", _settings())
        assert isinstance(strat, OpenDonchian)
    ...
    def test_parameters_are_class_constants(self):
        ...
        assert OpenDonchian(channel=33).channel == 33
...
class TestEvaluate:
    def test_breakout_in_clean_uptrend_emits_buy(self):
        intent = OpenDonchian().evaluate("TEST.EPIC", _buffer(_trending_up(60)))
    ...
    def test_choppy_regime_is_rejected_by_efficiency_gate(self):
        strat = OpenDonchian(min_efficiency=0.45)
    ...
    def test_wide_spread_is_rejected(self):
        ...
        assert OpenDonchian().evaluate("TEST.EPIC", buf) is None

    def test_insufficient_warmup_returns_none(self):
        assert OpenDonchian().evaluate("TEST.EPIC", _buffer(_trending_up(5))) is None

    def test_no_breakout_inside_band_returns_none(self):
        # Flat series: the last close never escapes the prior band.
        flat = [8000.0] * 60
        assert OpenDonchian().evaluate("TEST.EPIC", _buffer(flat)) is None

    def test_is_entry_strategy_instance(self):
        assert isinstance(OpenDonchian(), EntryStrategy)
